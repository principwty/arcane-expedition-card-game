...
const BASE_CARDS = [
  ...
  card("starlit-aegis", "星幕庇護", 2, "spell", "star", "common", null, "為一個友方角色恢復 3 點生命。", ["heal"], [{ type: "healTarget", amount: 3, target: "ally" }]),
  ...
  card("great-stag", "聖角巨鹿", 6, "minion", "forest", "rare", { attack: 4, health: 7, speed: 1 }, "守護。登場：為你的英雄恢復 4 點生命。", ["beast", "guard"], [{ type: "healHero", amount: 4 }]),
  ...
  card("ember-squire", "餘燼侍從", 1, "minion", "dragon", "common", { attack: 2, health: 1, speed: 2 }, "速度 2：登場回合即可攻擊。", ["soldier"], []),
  ...
  card("ash-runner", "灰燼奔襲者", 2, "minion", "dragon", "common", { attack: 3, health: 2, speed: 2 }, "速度 2。", ["soldier"], []),
  ...
  card("soul-siphon", "汲魂術", 3, "spell", "moon", "rare", null, "對一個敵方目標造成 2 點傷害，為你的英雄恢復 2 點生命。", ["damage"], [{ type: "damage", amount: 2, target: "enemy" }, { type: "healHero", amount: 2 }]),
  ...
  card("bulwark-sentinel", "壁壘哨衛", 3, "minion", "iron", "common", { attack: 2, health: 5, speed: 1 }, "守護。", ["soldier", "guard"], []),
  ...
  card("wandering-sprite", "流浪小精", 1, "minion", "neutral", "common", { attack: 1, health: 2, speed: 2 }, "速度 2。", ["beast"], []),
...
const EVOLUTION_CARDS = [
  evo("astral-ascension", "星界升格", 3, "spell", "star", "覺醒", "抽 2 張牌。你的遠征軌 +1。", [{ type: "draw", amount: 2 }, { type: "expedition", amount: 1 }], true),
  ...
  evo("verdant-sanctuary", "翠綠聖域", 4, "artifact", "forest", "覺醒", "每回合第一次召喚野獸時，為你的英雄恢復 2 點生命。", [{ type: "beastHealArtifact", amount: 2 }]),
  ...
  evo("dragon-crown", "龍王冠冕", 6, "artifact", "dragon", "覺醒", "你的傷害法術額外造成 2 點傷害。", [{ type: "spellDamage", amount: 2 }]),
  evo("apex-dragon", "終焰巨龍", 8, "minion", "dragon", "發現", "8/8。速度 2。登場：對敵方英雄造成 4 點傷害。", [{ type: "damageHero", amount: 4 }], false, { attack: 8, health: 8, speed: 2 }, ["dragon"]),
  ...
  evo("grave-covenant", "墓園契約", 4, "artifact", "moon", "覺醒", "每回合第一次友方召喚物死亡時，召喚 1 個 1/1 亡魂。", [{ type: "firstDeathWraith" }]),
  ...
  evo("colossus-frame", "巨像框架", 7, "minion", "iron", "發現", "7/9。守護。你的遺物不會被摧毀。", [], false, { attack: 7, health: 9, speed: 1 }, ["guard"]),
  evo("arsenal-awakening", "軍械覺醒", 4, "artifact", "iron", "覺醒", "你的召喚物每回合首次攻擊後，使其 +1/+1。", [{ type: "firstAttackBuff", attack: 1, health: 1 }]),
...
function playCard(playerIndex, cardUid, target = null) {
  ...
  if (played.type === "minion") {
    ...
    resolveEffects(playerIndex, played.effects.filter((effect) => effect.type === "damageHero"), played, target);
...
function summonMinion(playerIndex, minion, target = null) {
  ...
  minion.canAttack = minion.stats.speed >= 2;
...
function resolveAttack(playerIndex, attackerUid, targetUid = null) {
  ...
  if (targetUid) {
    ...
    if (!target) return;
    target.currentHealth -= totalAttack;
    attacker.currentHealth -= target.currentAttack;
    if (target.currentHealth <= 0) killMinion(1 - playerIndex, target.uid, attacker);
